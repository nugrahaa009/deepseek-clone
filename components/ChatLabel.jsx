...
const ChatLabel = ({ openMenu, setOpenMenu, id, name }) => {
  ...
  const renameHandler = async () => {
    try {
      ...
      if (!newName) return;
      const { data } = await axios.put('/api/chat/rename', { chatId: id, name: newName });
      if (data.success) {
        fetchUserChats();
  ...
  return (
    ...
    >
      ...
      >
        <Image className={`w-4 ${openMenu.id === id && openMenu.open ? 'block' : 'hidden'} group-hover:block`} src={assets.three_dots} alt='' />
